...
void AsmGenerator::generateArithmetic(Quad& q) {
    ...
    if (opcode == OpCode::ASSIGN) {
        ...
        if (flag == 7) {
            std::string tempVar = q.getArg(1).var->getName();
            if (tempVar[0] == 'T') {
    ...
    std::string resultIdName = q.getArg(3).var->getName();
    
    // All args and reslut is var or temp var.
    if (flag == 7) {
        arg1IdName = q.getArg(1).var->getName();
        arg2IdName = q.getArg(2).var->getName();
        if (arg1IdName[0] == 'T') {
...
//     }
//     else {
//         std::string argNam = arg.getArg(1).var->getName();
...
//         std::string tempVar = q.getArg(3).var->getName();
...
void AsmGenerator::generateSetArg(Quad& q) {
    ...
    std::string varName = "";
    if (flag == 7) {
        varName = q.getArg(1).var->getName();
        if (varName[0] != 'T') {
            int offset = q.getArg(1).var->getOffset();
...
void AsmGenerator::generateJump(Quad& q) {
    ...
    if (opcode == OpCode::JUMP) {
        ...
        if (flag == 0) {
            ...
            std::string var1Name = q.getArg(1).var->getName();
            if (var1Name[0] == 'T') {
            ...
            std::string var2Name = q.getArg(2).var->getName();
            if (var2Name[0] == 'T') {
...
void AsmGenerator::generateNeg(Quad& q) {
    std::string varName = q.getArg(1).var->getName();
    std::string result = q.getArg(3).var->getName();
...
void AsmGenerator::generatePower(Quad& q) {
    ...
    if (flag == 7) {
        ...
        std::string arg1Name = q.getArg(1).var->getName();
        if (arg1Name[0] == 'T') {
        ...
        std::string arg2Name = q.getArg(2).var->getName();
        if (arg2Name[0] == 'T') {
...
void AsmGenerator::generateGetAddress(Quad& q) {
    int offset = q.getArg(1).var->getOffset();
    std::string resultName = q.getArg(3).var->getName();
    if (resultName[0] == 'T') {
...
//     int flag = q.getFlag();
//     if (flag == 7) {
//         std::string tempValue = q.getArg(1).var->getName();
//         asmRegister reg;
//         if (tempValue[0] == 'T') {
...
//     std::string tempResult = q.getArg(3).var->getName();
//     std::string structIdName = q.getArg(1).var->getName();
//     int offsetOfMember = std::atoi(q.getArg(2).var->getName().c_str());
//     int offsetOfStruct = currentTable->findSymbol(structIdName)->getOffset();
...
void AsmGenerator::generateGetArrayValue(Quad& q) {
    ...
    if (q.getOpCode() == OpCode::GET_ARRAY) {
        ...
        int flag = q.getFlag();
        if (flag == 7) {
            std::string offsetName = q.getArg(2).var->getName();
            if (offsetName[0] == 'T') {
...
void AsmGenerator::generateAssignArray(Quad& q) {
    ...
    if (q.getOpCode() == OpCode::ASSIGN_ARRAY) {
        ...
        if (offsetName[0] == 'T') {
            ...
            int offset2 = std::atoi(q.getArg(2).var->getName().c_str()) * 4;
            baseOffset += offset2;
        } else {
            int offset = q.getArg(2).var->getOffset();
            offsetEbpStr = this->asmcode.generateVar(offset);
